...
2. Start the server (SQLite is used as a lightweight SQL database)
...
If `npm install` fails due to native build tools on Windows, try installing the prebuilt `sqlite3` package or install the required build tools (Visual Studio Build Tools). Tell me and I will switch the code to use `sqlite3` instead.
